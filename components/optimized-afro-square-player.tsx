...
import { musicPlayerService } from "@/lib/music-player-service"
import type { AfroSquareTrack } from "@/lib/afro-square-tracks"
import Image from "next/image"

interface OptimizedAfroSquarePlayerProps {
...
export default function OptimizedAfroSquarePlayer({ tracks, onDownloadClick }: OptimizedAfroSquarePlayerProps) {
  const playerId = "afro-square-player"
  const compilation = "afro-square"
  ...
  // Memoized current track for performance
  ...
  useEffect(() => {
    ...
    // Preload first few tracks for better performance
    musicPlayerService.preloadCompilationAudio(compilation, 3)
  }, [])

  // Sync with music player service
  useEffect(() => {
    musicPlayerService.updatePlayerState(playerId, {
  ...
  }, [currentTrack?.id, isPlaying, currentTime, duration, volume, shuffle, repeat, playHistory])
  ...
  const handleNext = () => {
    ...
    // Preload next track for smoother experience
    const nextTrack = tracks[nextIndex]
    if (nextTrack) {
      musicPlayerService.preloadAudio(nextTrack.audioUrl)
    }
