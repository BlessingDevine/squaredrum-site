...
import type { CountrySquareTrack } from "@/lib/country-square-tracks"
import Image from "next/image"

interface CountrySquarePlayerProps {
  tracks: CountrySquareTrack[]
...
export default function CountrySquarePlayer({ tracks, onDownloadClick }: CountrySquarePlayerProps) {
  ...
  return (
    <div className="bg-gradient-to-br from-amber-900/20 to-yellow-900/20 backdrop-blur-sm border border-amber-500/20 rounded-xl p-6">
      <audio ref={audioRef} src={currentTrack.audioUrl} preload="metadata" />

      {/* Header */}
      <div className="flex items-center justify-between mb-6">
        <div>
          <h3 className="font-cinzel text-xl font-bold text-amber-400 mb-1">COUNTRY SQUARE</h3>
          <p className="text-gray-400 text-sm">{tracks.length} tracks • 2025</p>
        </div>
        <Button onClick={handleDownload} size="sm" className="bg-amber-600 hover:bg-amber-700 text-white">
      ...
      <div className="flex items-center space-x-4 mb-6">
        ...
        <div className="flex-1 min-w-0">
          ...
          <p className="text-amber-400 text-sm truncate">{currentTrack.artist}</p>
      ...
      <div className="flex items-center justify-center space-x-1 mb-4">
        <Button
          ...
          className={`text-gray-400 hover:text-white ${shuffle ? "text-amber-400" : ""}`}
          title="Shuffle"
        >
        ...
        <Button
          ...
          className="text-white hover:text-amber-400 mx-2"
          disabled={isLoading}
        >
          {isLoading ? (
        ...
        <Button
          ...
          className={`text-gray-400 hover:text-white ${repeat !== "none" ? "text-amber-400" : ""}`}
          title={`Repeat: ${repeat}`}
        >
